pipeline {
    ...
    stages {
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=ns-application \
                    -Dsonar.projectKey=ns-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Building"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                            sh "docker build -t notes-webapp ." 
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag notes-webapp yash5090/notes-webapp:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/notes-webapp:latest" 
            } 
        } 
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/notes-webapp:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/notes-webapp:latest'
                       sh 'docker-scout cves yash5090/notes-webapp:latest'
                       sh 'docker-scout recommendations yash5090/notes-webapp:latest'
                       sh 'docker-scout attestation yash5090/notes-webapp:latest'
                   }
                }   
            }
        }
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/notes-webapp:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name notes-webapp -p 5000:80 yash5090/notes-webapp:latest' 
                //}
            }
        }
